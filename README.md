# CLover — 로컬 AI 기반 생활 돌봄 앱

> **2026-09-25 개편 구현:** 어르신 단일 마이크 → 저장형 음성 작업 → 일반 일정 자동 확정 → 예외만 사회복지사 조율 흐름을 추가했습니다. [업데이트·설정·현재 제한](docs/FLOW-V2.md), [기획서](docs/SERVICE-PLAN.md), [Codex 인계](docs/CODEX-HANDOFF.md)를 먼저 확인하세요.

**Android·iPhone 앱 + 기관 PC의 로컬 AI 서버 구성입니다.** 모바일 실행 순서는 [앱 개발 안내](docs/MOBILE.md)를 참고하세요. 로그인·가입 승인·음성/글 접수·담당자 검토·지역/일정 연결·안내문 화면을 제공합니다.
...
새 마이크 화면은 요청 저장과 배정 결과를 음성으로 알립니다. 기존 자동 안부 질문은 새 화면에 아직 연결되지 않았습니다. 아래 음성 확인 문서는 이전 화면의 동작 기록입니다. [설정과 제한 사항](docs/VOICE-CHECKIN.md)을 확인하세요.

## 담당 팀과 이동 시간 및 처리 이력

관리자가 요양보호사별 담당 사회복지사를 연결하고, 담당 팀 안에서 일정을 제안합니다. 선택적으로 네이버 주소 검색·자동차 길찾기를 연결해 앞뒤 방문 이동 시간을 검사합니다. 관리자·사회복지사용 처리 이력도 추가했습니다. [설정과 사용법](docs/MAPS-TEAMS.md)을 참고하세요. 지도 API 키는 기관 서버에서 별도 설정합니다.

## 이전 접수 업무함 (기존 자료 관리용)

AI가 접수 요약·원문 근거·전달문을 먼저 준비하고 사회복지사가 AI 업무함에서 확인합니다. 일반 요청의 묶음 승인과 개입 대상 구분, 승인된 돌봄 전달문, 승인·수정 건수 집계를 추가했습니다. [부족했던 점과 개선 내용](docs/AI-WORKFLOW-IMPROVEMENTS.md)을 참고하세요.

## 기획서와 기존 3단계 일정 조율

[서비스 기획서](docs/SERVICE-PLAN.md)에 현재 구현 범위와 남은 과제를 정리했습니다. [3단계 일정 안내](docs/SCHEDULING.md)에 자동 제안, 남은 시간 조율, 긴급 제안과 요양보호사 수락·거절 절차를 설명합니다.

## 역할·통계·반복 요청 업데이트

사회복지사는 일정 조율과 요청 검토, 요양보호사는 직접 돌봄 수행을 담당합니다. [역할 및 자동화 안내](docs/ROLES-AND-AUTOMATION.md)에 통계 기준, 반복 요청 검토 흐름과 DB 업데이트 절차를 정리했습니다.

## 음성·지역 일정 연결 업데이트

기존 사용자는 [업데이트 및 음성 설치 안내](docs/UPGRADE-VOICE-MATCHING.md)를 먼저 확인하세요. 음성 인식은 별도 tiny 모델 설치가 필요하며, AI 분석 동의는 최초 한 번 저장하고 설정에서 변경합니다.
...
398MB는 다운로드 크기이며 실행 메모리 총량이 아닙니다. Python·Ollama·KV cache·OS 메모리는 별도로 필요합니다. CPU로 실행할 수 있지만 사용자 PC의 속도와 메모리 사용량은 아직 측정하지 않았습니다. 작은 모델이므로 한국어 이해·판단·문장 품질이 큰 모델보다 제한될 수 있습니다. 임상적으로 검증된 분류기가 아니며 위험·불확실한 요청은 담당자가 확인합니다. 명확한 일반 요청만 사전 동의 범위에서 자동 배정합니다.

## 가볍게 실행하도록 변경한 점

- 모델 하나를 고정 사용: `qwen2.5:0.5b`.
- 컨텍스트 2,048 토큰, 분류·요약·근거 출력 최대 384 토큰, 안내문 최대 384 토큰.
...
`allow_local_ai`를 생략하면 저장된 AI 설정을 사용합니다. 설정하지 않은 계정은 분석하지 않습니다. `allow_local_ai=false`여도 접수되며 사람이 확인합니다. 과거 `allow_structured_ai` 필드는 호환 목적으로 받되 무시하며 동의로 재사용하지 않습니다. 긴급 키워드와 선택 항목은 모델 없이 우선 확인합니다.
...
- 회원 승인, 역할/담당자별 접근 제어, 원문 Fernet 암호화, 최소 감사 기록, 일일 이용 확인, 최근 반복 요청 집계 유지.
- 운영에는 TLS·접근 통제·속도 제한·DB/백업 보호 필요. 모델 신뢰도 수치는 실제 정확도 보장이 아님.
- 응급 모델이 아니며 119 자동 신고 기능 없음. 담당자가 요청을 확인하는 절차 필요.
- 다기관 격리, 보호자 대리 관계, 요양보호사 평점·비용 정산은 아직 미구현. 지역·일정 신청/사회복지사 제안·변경/요양보호사 수락·거절/요양보호사 수행·보고는 지원합니다.
