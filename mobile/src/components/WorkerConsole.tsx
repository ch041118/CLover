...
type Schedule=Slot&{id:string;elder_id:string;category:Category;status:string;worker_id:string|null;care_id:string|null;outcome:string|null};
export const states:Record<string,string>={pending:'조율 대기',accepted:'배정 확정',in_progress:'돌봄 진행',completed:'완료',attention:'특이사항 확인 필요',cancelled:'취소',declined:'기존 거절'};
function SlotLabel(slot:Slot){return `${slot.day} ${slot.start}–${slot.end} · ${slot.caregiver_id} · ${slot.province} ${slot.district}`;}
function ContactCheck({value,onChange}:{value:boolean;onChange:(v:boolean)=>void}){return <View style={s.switchRow}><Switch accessibilityLabel="당사자와 일정 확인 완료" value={value} onValueChange={onChange}/><Text style={[s.body,{flex:1}]}>이용자·요양보호사와 해당 일정을 확인했습니다.</Text></View>;}
...
export function RequestScheduler({api,careId,reviewRequired}:{api:ApiClient;careId:string;reviewRequired:boolean}){
  ...
  return <View style={{gap:12}}><Text style={s.heading}>요양보호사 배정</Text><Text style={s.body}>분류를 검토한 후 이용자와 같은 지역의 근무 가능 일정에 연결합니다.</Text>
    ...
    {selected&&<><ContactCheck value={confirmed} onChange={setConfirmed}/><Button disabled={task.busy||!confirmed||reviewRequired} title="이 요청에 돌봄 일정 배정" onPress={()=>task.run(async()=>{await api.request('/api/worker/assign',{care_id:careId,slot_id:selected,contact_confirmed:confirmed});setSlots([]);setSelected('');task.setMessage('배정했습니다. 일정 관리에서 변경·확인할 수 있습니다.');})}/></>}
...
function ScheduleCard({api,row,userId,onChanged}:{api:ApiClient;row:Schedule;userId:string;onChanged:()=>Promise<void>}){
  ...
  return <Card><Text style={s.label}>{states[row.status]||row.status} · {categories[row.category]}</Text><Text style={s.body}>{row.elder_id} → {row.caregiver_id}{'\n'}{row.day} {row.start}–{row.end}{'\n'}{row.province} {row.district}{'\n'}관리 담당: {row.worker_id||'미배정'}</Text>
    ...
    {row.worker_id===userId&&<>
      {row.status==='pending'&&<><ContactCheck value={confirmed} onChange={setConfirmed}/><Button disabled={task.busy||!confirmed} title="조율 완료 · 일정 확정" onPress={()=>task.run(async()=>{await api.request(`/api/worker/schedules/${row.id}/confirm`,{contact_confirmed:confirmed});await onChanged();})}/></>}
      {['pending','accepted'].includes(row.status)&&<><Button secondary disabled={task.busy} title="변경 가능한 일정 보기" onPress={()=>task.run(async()=>{const all=await api.request<Slot[]>('/api/worker/open-slots');setSlots(all.filter(x=>x.province===row.province&&x.district===row.district));setTarget('');setConfirmed(false);if(!all.length)task.setMessage('근무 가능 일정이 없습니다.');})}/>
      ...
      {target&&<><ContactCheck value={confirmed} onChange={setConfirmed}/><Button disabled={task.busy||!confirmed} title="담당자·시간 변경 확정" onPress={()=>task.run(async()=>{await api.request(`/api/worker/schedules/${row.id}/reschedule`,{slot_id:target,contact_confirmed:confirmed});setSlots([]);setTarget('');await onChanged();})}/></>}
...
export function RepeatInbox({api,userId}:{api:ApiClient;userId:string}){
  ...
  return <><Card><Text style={s.heading}>반복 요청 · 사람이 개입할 지점</Text><Text style={s.body}>같은 어르신·같은 종류 요청이 7일 안에 3회 이상이면 자동으로 묶습니다. 같은 문장이라는 뜻은 아니므로 개별 내용을 확인하세요.</Text><Text style={s.caption}>자동: 묶음 생성, 승인한 담당자 배정, 확인일 표시. 사람: 상담·위험도 확인·정기 돌봄 계획·일정 확정.</Text><Button secondary disabled={task.busy} title="새로고침" onPress={()=>task.run(load)}/><Button secondary disabled={task.busy} title="업데이트 전 요청도 찾아 묶기" onPress={()=>task.run(async()=>{await api.request('/api/worker/repeats/scan',{});await load();})}/><Notice text={task.message}/>{!rows.length&&<Text style={s.body}>현재 검토할 반복 요청 묶음이 없습니다.</Text>}</Card>{rows.map(row=><RepeatCard key={`${row.id}:${row.state}:${row.worker_id}`} api={api} row={row} userId={userId} onChanged={load}/>)}</>;
}
