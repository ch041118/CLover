...
export function RequestScheduler({api,careId,reviewRequired}:{api:ApiClient;careId:string;reviewRequired:boolean}){
  ...
  return <View style={{gap:12}}><Text style={s.heading}>2·3차 일정 제안</Text><Text style={s.body}>일반 요청은 2차 빈 시간 조율, 긴급 분류 요청은 3차 긴급 제안으로 전달됩니다. 모든 일정은 요양보호사가 수락해야 확정됩니다.</Text>
    ...
    {loaded&&!slots.length&&<Text style={s.body}>담당 팀에 등록된 요양보호사 중 이용자 지역과 일치하는 일정이 없습니다. 이용자 지역과 요양보호사 근무 일정을 먼저 등록해 주세요.</Text>}
    ...
    {selected&&<><Button secondary disabled={task.busy} title="선택한 일정의 이동 시간 확인" onPress={()=>task.run(async()=>{const r=await api.request<{mode:string;inbound_minutes?:number;outbound_minutes?:number;warning:string}>('/api/worker/route-check',{care_id:careId,slot_id:selected});task.setMessage(r.mode==='naver_driving'?`이전 장소에서 ${r.inbound_minutes}분 / 다음 방문까지 ${r.outbound_minutes}분 (여유 시간 포함). ${r.warning}`:r.warning);})}/><ContactCheck value={confirmed} onChange={setConfirmed}/><Button disabled={task.busy||!confirmed||reviewRequired} title="요양보호사에게 일정 제안" onPress={()=>task.run(async()=>{await api.request('/api/worker/assign',{care_id:careId,slot_id:selected,contact_confirmed:confirmed});setSlots([]);setSelected('');task.setMessage('제안했습니다. 요양보호사 수락 후 확정됩니다.');})}/></>}
...
export function RepeatInbox({api,userId}:{api:ApiClient;userId:string}){
  ...
  return <><Card><Text style={s.heading}>반복 요청 · 사람이 개입할 지점</Text><Text style={s.body}>같은 어르신·같은 종류 요청이 7일 안에 3회 이상이면 자동으로 묶습니다. 같은 문장이라는 뜻은 아니므로 개별 내용을 확인하세요.</Text><Text style={s.caption}>자동: 묶음 생성, 승인한 담당자 배정, 확인일 표시. 사람: 상담·위험도 확인·정기 돌봄 계획·일정 확정.</Text><Button disabled={task.busy} title="1차 · 향후 7일 자동 제안 실행" onPress={()=>task.run(async()=>{const r=await api.request<{offered:string[];skipped:{care_id:string;reason:string}[]}>('/api/worker/scheduler/run',{});await load();task.setMessage(`제안 ${r.offered.length}건 / 미연결 ${r.skipped.length}건`+r.skipped.map(x=>`\n${x.care_id}: ${x.reason}`).join(''));})}/><Text style={s.caption}>내가 검토한 미연결 일반 요청을 접수순으로 최대 100건 처리합니다. 지역이 같은 빈 시간 중 지도 연동 시 앞뒤 이동 시간을 검증한 일정에 제안하며, 거절된 건은 자동 재제안하지 않습니다.</Text><Button secondary disabled={task.busy} title="새로고침" onPress={()=>task.run(load)}/><Button secondary disabled={task.busy} title="업데이트 전 요청도 찾아 묶기" onPress={()=>task.run(async()=>{await api.request('/api/worker/repeats/scan',{});await load();})}/><Notice text={task.message}/>{!rows.length&&<Text style={s.body}>현재 검토할 반복 요청 묶음이 없습니다.</Text>}</Card>{rows.map(row=><RepeatCard key={`${row.id}:${row.state}:${row.worker_id}`} api={api} row={row} userId={userId} onChanged={load}/>)}</>;
}
